...
class LangSegment:
    ...
    def _saveData(words, language: str, text: str):
        ...
        if not "|" in language:
            lang_count[language] += (
                int(len(text) // 2) if language == "en" else len(text)
            )
    ...
    def _addwords(words, language, text):
        ...
        if preResult and ("|" in preResult["lang"]):
            ...
            if ispre_waits:
                preResult = LangSegment._saveData(
                    words, preResult["lang"], preResult["text"]
                )
        ...
        filters = LangSegment.Langfilters
        if "|" in language:
            LangSegment._text_waits.append({"lang": language, "text": text})
        else:
    ...
    def _parse_language(words, segment):
        ...
        for index, text in enumerate(lines):
            ...
            if len(cleans_text) <= 3 and LangSegment._is_chinese(cleans_text):
                ...
                elif LangSegment._is_japanese_kana(cleans_text):
                    language = LANG_JA
                else:
    ...
    def getCounts():
        ...
        for d in text_langs:
            lang_counts[d["lang"]] += (
                int(len(d["text"]) // 2) if d["lang"] == "en" else len(d["text"])
            )
        lang_counts = dict(
